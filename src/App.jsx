...
const NAV = [
  ...
  { to: '/app/signals',   label: 'Signals',   icon: '📡', k: '2' },
  { to: '/app/portfolio', label: 'Portfolio', icon: '💼', k: '3' },
  { to: '/app/journal',   label: 'Journal',   icon: '📓', k: '4' },
]
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('onboarded').eq('id', user.id).single()
      .then(({ data }) => { if (data && !data.onboarded) setShowOnboarding(true) }).catch(() => {})
    supabase.from('journal_entries').select('pnl, status').eq('user_id', user.id)
      .then(({ data }) => {
        if (!data) return
        ...
        setJournalStats({ totalTrades: closed.length, totalPnl, winRate })
      }).catch(() => {})
  }, [user])
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === '?') showToast('⌨️  Shortcuts: 1-4 navigate · [ sidebar · ? help')
    }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#080c14', overflow: 'hidden' }}>
      <style>{`
        ...
        .collapse-btn:hover { color:#8b9fc0!important; background:rgba(255,255,255,0.07)!important; }
      `}</style>

      <aside style={{ width: collapsed ? 60 : 220, background: '#090e18', borderRight: '1px solid rgba(255,255,255,0.055)', display: 'flex', flexDirection: 'column', flexShrink: 0, transition: 'width 0.22s cubic-bezier(0.4,0,0.2,1)', overflow: 'hidden', zIndex: 20 }}>
        <div style={{ padding: '15px 12px 13px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 56, flexShrink: 0 }}>
          ...
          <button className="collapse-btn" onClick={() => setCollapsed(c => !c)} title="[ to toggle"
        ...
        <nav className="sidebar-nav" style={{ flex: 1, padding: '12px 8px', display: 'flex', flexDirection: 'column', gap: 1, overflowY: 'auto' }}>
          {NAV.map(({ to, label, icon, end, k }) => (
        ...
        <div style={{ padding: '8px 8px 12px', borderTop: '1px solid rgba(255,255,255,0.05)', flexShrink: 0 }}>
          ...
          {!collapsed && (
            <>
              ...
              <div style={{ color: '#1a2535', fontSize: 9, fontFamily: '"DM Mono",monospace', textAlign: 'center', marginTop: 8, letterSpacing: '0.04em' }}>? for shortcuts</div>
            </>
          )}
        </div>
      </aside>
      ...
      <AIChat journalStats={journalStats} />
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: '#111927', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '10px 20px', color: '#e2e8f0', fontSize: 12, fontFamily: '"DM Mono",monospace', zIndex: 1000, animation: 'toastIn 0.2s ease', boxShadow: '0 8px 32px rgba(0,0,0,0.4)', whiteSpace: 'nowrap' }}>
          {toast}
        </div>
      )}
    </div>
