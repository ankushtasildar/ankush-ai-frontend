...
export default function Overview() {
  ...
  useEffect(() => {
    fetch('/api/market-scanner?action=scan').then(r=>r.json()).then(d=>{
      setScanResults(d);
      if(d&&d.opportunities&&d.opportunities.length>0)fetch('/api/day-trade-engine?action=predict&symbol='+d.opportunities[0].symbol).then(r2=>r2.json()).then(v3=>setDeepAlert(v3)).catch(()=>{});
    }).catch(()=>{});
  }, []);
  const sp = mkt?.spy
  const qq = mkt?.qqq
          ...
          {deepAlert&&deepAlert.alert&&(
            <div style={{background:deepAlert.alert.direction==="BULLISH"?"#0f3d0f":"#3d0f0f",borderRadius:12,padding:"14px 16px",marginBottom:10,border:"1px solid "+(deepAlert.alert.direction==="BULLISH"?"#1a6b1a":"#6b1a1a")}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                <span style={{background:"#166534",color:"#fff",padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:600}}>Grade {deepAlert.alert.grade}</span>
              </div>
              ...
              <div style={{marginTop:6,fontSize:10,color:"#6b7280"}}>{deepAlert.alert.confluencePct}% confluence | {deepAlert.alert.timeframe}</div>
            </div>
          )}
