...
// Setups that ALWAYS show while live data loads — based on structural market knowledge
const BASELINE_SETUPS = [
  { symbol: 'SPY', setupType: 'Index regime read — EMA stack + VIX positioning', bias: 'neutral', confidence: 7, optionsTrade: 'Monitor SPY weekly straddle near key levels', entry: 'At key S/R', target: '+2-3%', stop: '-1.5%', keyFactor: 'SPY structure dictates the entire market regime. VIX below 20 favors call buying on dips; above 25 favors put spreads on rips.', frameworks: ['macro', 'technical', 'options'], urgency: 'high' },
  { symbol: 'QQQ', setupType: 'Tech leadership — momentum + EMA confluence', bias: 'bullish', confidence: 7, optionsTrade: 'Buy QQQ ATM calls 3-4 weeks out on EMA21 reclaim', entry: 'EMA21 reclaim', target: 'Prior highs', stop: 'Below EMA50', keyFactor: 'QQQ is the risk-on thermometer. When tech leads with EMA stack intact, buy dips to EMA21. When below EMA50, shift to spreads only.', frameworks: ['momentum', 'technical', 'the_strat'], urgency: 'medium' },
  { symbol: 'NVDA', setupType: 'AI earnings cycle + options IV expansion setup', bias: 'bullish', confidence: 8, optionsTrade: 'Buy NVDA calls 4-6 weeks before earnings when IV rank < 50%', entry: 'EMA21 support', target: '61.8% extension', stop: 'Below EMA50', keyFactor: 'NVDA has consistently moved 8-15% on earnings. With AI infrastructure demand, pre-earnings IV expansion is a repeatable edge. Buy when IV rank is compressed.', frameworks: ['earnings', 'options', 'momentum'], urgency: 'high' },
  { symbol: 'AAPL', setupType: 'Sympathy + supplier chain reaction play', bias: 'neutral', confidence: 6, optionsTrade: 'Buy AAPL ATM calls week of iPhone/product events', entry: 'Pre-event consolidation', target: '+5-8%', stop: 'Day low break', keyFactor: 'AAPL product events create sympathy moves across suppliers (TSMC, Skyworks, Cirrus). AAPL itself often pulls back post-announcement — trade the setup, not the news.', frameworks: ['sympathy', 'earnings', 'technical'], urgency: 'medium' },
  { symbol: 'PLTR', setupType: 'Government AI contract + breakout above consolidation', bias: 'bullish', confidence: 7, optionsTrade: 'Buy PLTR $X calls 3 weeks out on volume breakout confirmation', entry: 'Break of 52W high on 2x volume', target: '127% fib extension', stop: 'Back below breakout level', keyFactor: 'PLTR wins government AI/defense contracts in cycles. High short interest means breakouts are violent. Wait for volume confirmation — fake breakouts are common.', frameworks: ['breakout', 'technical', 'macro'], urgency: 'medium' },
  { symbol: 'AMD', setupType: 'NVDA earnings sympathy + semiconductor rotation', bias: 'bullish', confidence: 7, optionsTrade: 'Buy AMD calls day before NVDA earnings report', entry: 'Morning of NVDA earnings', target: '+6-10% in 2 days', stop: 'If NVDA disappoints', keyFactor: 'AMD is the most reliable sympathy play to NVDA earnings. When NVDA beats, AMD gaps up 4-8% next day as traders re-rate the entire AI chip sector.', frameworks: ['sympathy', 'earnings', 'sector'], urgency: 'high' },
]
...
function SetupCard({ setup, index, isLive }) {
  ...
  return (
    <div onClick={() => setExpanded(e => !e)} style={{ background: '#0d1420', border: '1px solid ' + (expanded ? bias.border : 'rgba(255,255,255,0.07)'), borderRadius: 16, padding: '18px 20px', cursor: 'pointer', transition: 'border-color .2s', position: 'relative', overflow: 'hidden' }}>
      ...
      {!isLive && <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 4, padding: '2px 6px', fontSize: 9, color: '#f59e0b', fontFamily: '"DM Mono",monospace' }}>BASELINE</div>}
...
export default function TopSetups() {
  ...
  const REFRESH_MS = 5 * 60 * 1000 // 5 minutes between full AI scans (AI takes ~20-30s per scan)
  ...
  const runScan = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (d.setups && d.setups.length > 0) {
        ...
        setScanStatus('Live — ' + d.setups.length + ' setups found across ' + d.scanned + ' symbols')
      }
    } catch (err) {
      // On error, keep showing existing setups (baseline or last live)
      setScanStatus(isLive ? 'Live (cached) — retrying...' : 'Using baseline intelligence — live scan queued')
    } finally {
      setScanning(false)
    }
  }, [scanning, isLive])
  ...
  useEffect(() => {
    countdownRef.current = setInterval(() => {
      if (nextScan) {
        const remaining = Math.max(0, Math.round((nextScan - Date.now()) / 1000))
  ...
  // Initial scan + auto-refresh every 5 minutes
  useEffect(() => {
    runScan()
  ...
  return (
    <div style={{ padding: '24px 28px', minHeight: '100vh', background: '#080c14', color: '#f0f4ff', fontFamily: '"DM Sans",sans-serif' }}>
      <style>{`
        ...
        .fwbtn { background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.07); border-radius: 7px; padding: 5px 12px; color: #8b9fc0; font-family: "DM Mono",monospace; font-size: 10px; cursor: pointer; transition: all .15s; }
        .fwbtn:hover, .fwbtn.act { background: rgba(37,99,235,.12); border-color: rgba(37,99,235,.35); color: #60a5fa; }
        .setup-card { animation: slidein .3s ease; }
      `}</style>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 14 }}>
        <div>
          ...
          <p style={{ color: '#8b9fc0', fontSize: 13, margin: 0, maxWidth: 500 }}>100 analyst frameworks running in parallel across every timeframe, every strategy, every market condition.</p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
          ...
          {!scanning && nextScan && countdown !== null && (
            <div style={{ color: '#2d3d50', fontSize: 10, fontFamily: '"DM Mono",monospace' }}>
      ...
      <div style={{ display: 'flex', gap: 20, marginBottom: 18, flexWrap: 'wrap' }}>
        {[
          ['SETUPS', setups.length, '#f0f4ff'],
          ['HIGH CONF (8+)', setups.filter(s => s.confidence >= 8).length, '#10b981'],
          ['BULLISH', setups.filter(s => s.bias === 'bullish').length, '#10b981'],
          ['BEARISH', setups.filter(s => s.bias === 'bearish').length, '#ef4444'],
          ['SCANS TODAY', scanCount, '#60a5fa'],
        ].map(([l, v, c]) => (
          <div key={l}>
            <div style={{ color: '#2d3d50', fontSize: 9, fontFamily: '"DM Mono",monospace', letterSpacing: '.07em', marginBottom: 2 }}>{l}</div>
            <div style={{ color: c, fontSize: 22, fontWeight: 700, fontFamily: '"DM Mono",monospace', lineHeight: 1 }}>{v}</div>
          </div>
        ))}
        {lastScan && <div>
          <div style={{ color: '#2d3d50', fontSize: 9, fontFamily: '"DM Mono",monospace', letterSpacing: '.07em', marginBottom: 2 }}>LAST SCAN</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(360px,1fr))', gap: 14 }}>
        {filtered.map((setup, i) => (
          <div key={setup.symbol + i} className="setup-card">
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '40px', color: '#4a5c7a' }}>No {filter} setups in current scan. Try a different framework filter.</div>
      )}

      {/* Disclaimer */}
      <div style={{ marginTop: 28, padding: '12px 16px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ color: '#2d3d50', fontSize: 11, lineHeight: 1.6 }}>
          ⚠️ AnkushAI Top Setups are AI-generated analysis synthesizing technical, macro, and fundamental frameworks. Not financial advice. Options involve significant risk including potential total loss of premium.
        </div>
      </div>
    </div>
  )
