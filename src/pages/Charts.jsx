...
function ema(data,p){const k=2/(p+1);let e=data[0]?.close;return data.map((d,i)=>{if(i===0)return{time:d.time,value:e};e=d.close*k+e*(1-k);return{time:d.time,value:parseFloat(e.toFixed(4))}})}
function vwap(data){let cPV=0,cV=0;return data.map(c=>{const tp=(c.high+c.low+c.close)/3;cPV+=tp*(c.volume||1);cV+=(c.volume||1);return{time:c.time,value:parseFloat((cPV/cV).toFixed(4))}})}

function Section({title,icon,children,open:initOpen=false}){
  ...
  return(
    <div style={{borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:'100%',background:'none',border:'none',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',color:'#8b9fc0'}}>
        <span style={{fontFamily:'"DM Mono",monospace',fontSize:10,letterSpacing:'.08em',display:'flex',alignItems:'center',gap:6}}><span>{icon}</span>{title}</span>
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try{
        ...
        const r=await fetch('/api/analysis?type=analyze&symbol='+symbol,{headers:{'Authorization':'Bearer '+session?.access_token}})
        const reader=r.body.getReader();const dec=new TextDecoder();let txt=''
        while(true){
          const{done,value}=await reader.read();if(done)break
          dec.decode(value).split('
').filter(l=>l.startsWith('data: ')).forEach(l=>{
            try{const d=JSON.parse(l.slice(6));if(d.type==='data'){setData(d.tickerData);setLoading(false)}else if(d.type==='text'){txt+=d.text;setAnalysis(txt)}else if(d.type==='error')setError(d.error)}catch(e){}
          })
        }
  ...
  return(
    <div style={{width:340,borderLeft:'1px solid rgba(255,255,255,0.07)',background:'#090e18',display:'flex',flexDirection:'column',flexShrink:0,height:'100%',overflowY:'auto'}}>
      ...
      {loading&&!td&&<div style={{padding:20}}>
        <div style={{color:'#4a5c7a',fontSize:12,fontFamily:'"DM Mono",monospace',marginBottom:12}}>⚡ Loading market intelligence...</div>
        {['Fetching price data','Calculating indicators','Loading options chain','Running AI synthesis'].map((s,i)=>(
          <div key={s} style={{color:'#2d3d50',fontSize:11,padding:'4px 0',animation:'pulse 1.5s infinite',animationDelay:i*.4+'s'}}>▸ {s}</div>
        ))}
      </div>}
      ...
      {td&&<>
        <div style={{padding:'12px 16px',background:'rgba(255,255,255,0.02)'}}>
          <div style={{display:'flex',alignItems:'baseline',gap:8}}>
            ...
            <span style={{fontSize:13,color:td.changePct>=0?'#10b981':'#ef4444',fontFamily:'"DM Mono",monospace'}}>{td.changePct>=0?'▲':'▼'} {Math.abs(td.changePct||0).toFixed(2)}%</span>
          </div>
        ...
        <Section title="PRICE ACTION & TECHNICALS" icon="📈" open={true}>
        ...
        <Section title="FIBONACCI LEVELS" icon="🔢">
          {td.fibonacci&&Object.entries(td.fibonacci).map(([pct,price])=>{
            ...
            return(
              <div key={pct} style={{display:'flex',justifyContent:'space-between',padding:'4px 0',borderBottom:'1px solid rgba(255,255,255,0.03)',background:isNear?'rgba(245,158,11,0.05)':'transparent',margin:'0 -16px',paddingLeft:16,paddingRight:16}}>
                <span style={{color:isNear?'#f59e0b':'#4a5c7a',fontSize:11}}>{pct}</span>
                <span style={{color:isNear?'#f59e0b':'#8b9fc0',fontSize:11,fontFamily:'"DM Mono",monospace',fontWeight:isNear?700:400}}>${price?.toFixed(2)}{isNear?' ← NEAR':''}</span>
              </div>
            )
          })}
        </Section>
        {td.options?.expirations?.length>0&&<Section title="OPTIONS INTELLIGENCE" icon="⚡">
          <div style={{color:'#4a5c7a',fontSize:10,marginBottom:8}}>UPCOMING EXPIRATIONS</div>
          {(td.options.expirations||[]).slice(0,3).map(exp=><div key={exp} style={{color:'#8b9fc0',fontSize:11,fontFamily:'"DM Mono",monospace',padding:'2px 0'}}>{exp}</div>)}
        ...
        {td.news?.length>0&&<Section title="NEWS & SENTIMENT" icon="📰">
          {td.news.map((n,i)=>(
        ...
        <Section title="AI SYNTHESIS" icon="🧠" open={true}>
          {!analysis&&<div style={{color:'#4a5c7a',fontSize:12,fontFamily:'"DM Mono",monospace',animation:'pulse 1.5s infinite'}}>Running 100 analyst frameworks...</div>}
          {analysis&&<div style={{color:'#c4cfe0',fontSize:12,lineHeight:1.7,whiteSpace:'pre-wrap'}}>{analysis}</div>}
        </Section>
      </>}
...
export default function Charts(){
  ...
  function initChart(){
    ...
    chartRef.current=createChart(chartDiv.current,{layout:{background:{color:'#0d1420'},textColor:'#8b9fc0'},grid:{vertLines:{color:'rgba(255,255,255,0.04)'},horzLines:{color:'rgba(255,255,255,0.04)'}},crosshair:{mode:1},rightPriceScale:{borderColor:'rgba(255,255,255,0.1)'},timeScale:{borderColor:'rgba(255,255,255,0.1)',timeVisible:true,secondsVisible:false},handleScroll:true,handleScale:true})
    candleRef.current=chartRef.current.addCandlestickSeries({upColor:'#10b981',downColor:'#ef4444',borderUpColor:'#10b981',borderDownColor:'#ef4444',wickUpColor:'#10b981',wickDownColor:'#ef4444'})
  ...
  const fetchData=useCallback(async()=>{
    ...
    try{
      ...
      overlays.forEach(o=>{
        ...
        if(data.length){const s=chartRef.current.addLineSeries({color:COLS[o],lineWidth:1.5,priceLineVisible:false,lastValueVisible:true,title:o});s.setData(data);ovRef.current[o]=s}
      })
  ...
  useEffect(()=>{
    ...
    overlays.forEach(o=>{
      ...
      if(data.length&&chartRef.current){const s=chartRef.current.addLineSeries({color:COLS[o],lineWidth:1.5,priceLineVisible:false,lastValueVisible:true,title:o});s.setData(data);ovRef.current[o]=s}
    })
  },[overlays])
  ...
  function doSearch(v){
    ...
    stRef.current=setTimeout(async()=>{const r=await fetch('/api/market?type=search&q='+v).then(r=>r.json()).catch(()=>({results:[]}));setResults(r.results||[])},280)
  }
  ...
  return(
    <div style={{display:'flex',height:'100%',background:'#080c14',overflow:'hidden'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'20px 24px 16px',flexShrink:0}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16,gap:16,flexWrap:'wrap'}}>
            <div>
              ...
              {quote&&<div style={{display:'flex',alignItems:'center',gap:12}}>
                <span style={{fontSize:24,fontWeight:700}}>${quote.price?.toFixed(2)}</span>
                <span style={{color:chgCol,fontSize:13,fontFamily:'"DM Mono",monospace'}}>{isUp?'▲':'▼'} ${Math.abs(quote.change||0).toFixed(2)} ({Math.abs(quote.changePct||0).toFixed(2)}%)</span>
            ...
            <div style={{display:'flex',gap:10,alignItems:'center'}}>
              <div style={{position:'relative',minWidth:180}}>
                <input value={inp} onChange={e=>doSearch(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){setSym(inp.toUpperCase());setResults([])}}} placeholder="Search ticker..." style={{width:'100%',background:'#111927',border:'1.5px solid rgba(255,255,255,.1)',borderRadius:10,padding:'9px 14px',color:'#f0f4ff',fontSize:13,outline:'none',fontFamily:'"DM Mono",monospace',textTransform:'uppercase'}}/>
                {results.length>0&&<div style={{position:'absolute',top:'100%',insetInline:0,background:'#111927',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,zIndex:100,maxHeight:220,overflowY:'auto',marginTop:4,boxShadow:'0 8px 32px rgba(0,0,0,.5)'}}>
                  {results.map(r=><div key={r.symbol} className="sr" onClick={()=>{setSym(r.symbol);setInp(r.symbol);setResults([])}}><span style={{color:'#f0f4ff',fontFamily:'"DM Mono",monospace',fontWeight:600}}>{r.symbol}</span><span style={{color:'#4a5c7a',fontSize:11}}>{r.name?.substring(0,22)}</span></div>)}
                </div>}
              </div>
              <button onClick={()=>setShowPanel(p=>!p)} style={{padding:'9px 16px',background:showPanel?'rgba(37,99,235,0.15)':'rgba(255,255,255,0.04)',border:'1px solid '+(showPanel?'rgba(37,99,235,0.4)':'rgba(255,255,255,0.1)'),borderRadius:9,color:showPanel?'#60a5fa':'#8b9fc0',fontSize:12,cursor:'pointer',fontFamily:'"DM Mono",monospace',whiteSpace:'nowrap'}}>
        ...
        {candles.length>0&&<div style={{margin:'8px 24px 16px',background:'#0d1420',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,padding:'12px 16px',flexShrink:0}}>
          ...
          <div style={{display:'flex',alignItems:'flex-end',gap:1,height:48}}>
            {candles.slice(-60).map((c,i)=>{const mx=Math.max(...candles.slice(-60).map(x=>x.volume||0));const h=mx>0?(c.volume/mx)*48:0;const g=i>0&&c.close>=candles[candles.length-60+i-1]?.close;return <div key={i} style={{flex:1,height:h,background:g?'rgba(16,185,129,.4)':'rgba(239,68,68,.4)',borderRadius:'2px 2px 0 0',alignSelf:'flex-end',minWidth:1}}/>})}
          </div>
        </div>}
        {error&&<div style={{margin:'0 24px 16px',background:'rgba(239,68,68,.08)',border:'1px solid rgba(239,68,68,.2)',borderRadius:10,padding:'10px 14px',color:'#fca5a5',fontSize:12,flexShrink:0}}>{error}</div>}
      </div>
