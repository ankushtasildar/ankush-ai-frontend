...
const EMOTIONS = ['Confident','Cautious','FOMO','Fearful','Disciplined','Greedy','Patient','Anxious']
const SETUPS = ['Breakout','Breakdown','Mean Reversion','Trend Follow','Earnings Play','News Catalyst','Technical Pattern','Options Strategy']
    ...
    <div style={{ display:'flex',justifyContent:isAI?'flex-start':'flex-end',marginBottom:14 }}>
      <div style={{
        ...
        <div style={{ whiteSpace:'pre-wrap' }}>
          {msg.content.split('\n').map((line,i) => {
            const isBold = /^\*\*(.+)\*\*$/.test(line.trim()) || /^#{1,3}\s/.test(line)
...
function JournalChat({ entry, onClose }) {
  ...
  useEffect(() => {
    // Open with AI's proactive message about the trade
    if (!entry) return
    ...
    const initMsg = {
      role: 'assistant',
      ts: Date.now(),
      content: `Hey ${wasGain ? '— nice close today!' : '— tough one today.'} I saw you ${entry.status === 'closed' ? 'closed out your' : 'opened a'} ${entry.ticker} ${isOpts ? `${entry.strike} ${entry.optionType?.toUpperCase()}` : ''} position ${entry.status === 'closed' ? 'for a ' + (wasGain ? '+' : '') + (pnl ? '$'+Math.abs(pnl).toFixed(0) : 'flat') : 'entry'}.

${entry.status === 'closed' ? `Walk me through it — what was going through your head when you decided to ${wasGain ? 'take profits' : 'cut the position'}? Was it the chart, news, or something felt off?` : `Tell me about your thesis here. What's the setup you're playing?`}`
    }
    setMessages([initMsg])
  }, [entry])
  ...
  async function send() {
    ...
    const systemPrompt = `You are an elite trading journal coach and market analyst. Your role is to help traders learn from their decisions through Socratic questioning and market analysis.

TRADE CONTEXT:
Ticker: ${entry?.ticker} (${entry?.assetType || 'Stock'})
...
Strategy: ${entry?.strategy || 'Not specified'}
Entry Date: ${entry?.entryDate}
Status: ${entry?.status === 'closed' ? 'Closed' : 'Open/Active'}
P&L: ${pnl > 0 ? '+' : ''}$${pnl.toFixed(2)} (${wasGain ? 'WIN' : 'LOSS'})
Original Thesis: ${entry?.notes || 'Not recorded'}
Emotion at Entry: ${entry?.emotion || 'Not recorded'}
Setup: ${entry?.setup || 'Not recorded'}

YOUR COACHING STYLE:
- Ask probing questions to help the trader reflect on their decision-making process
- Reference specific market conditions, price action, or macro context if relevant
- If it was a good decision, reinforce WHY it was good — the process, not just the outcome
- If it was a poor decision, be honest but constructive — identify the specific cognitive bias or process failure
- Teach patterns: "This looks like classic [FOMO / fear / overconfidence / disciplined execution / etc.]"
- Connect their emotions to the outcome
- Never give explicit buy/sell signals for future trades
- Keep responses conversational — 3-5 sentences max unless they ask for a deep analysis
- Occasionally ask: "What would you do differently?" or "What signal told you to exit?"
- Be direct but empathetic — like a trading mentor, not a therapist`
    ...
    fetch('https://api.anthropic.com/v1/messages', {
      ...
      const reply = d.content?.[0]?.text || 'I had trouble connecting. Tell me more about the trade.'
      setMessages(p => [...p, { role:'assistant', ts:Date.now(), content:reply }])
      setLoading(false)
    }).catch(() => {
      setMessages(p => [...p, { role:'assistant', ts:Date.now(), content:'Connection issue — but tell me anyway. What was the trade-off you were weighing?' }])
      setLoading(false)
    })
  }
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:24 }}>
      <div style={{ background:'#0a0f1a',border:'1px solid #2d3f55',borderRadius:16,width:'100%',maxWidth:700,height:'80vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,0.8)' }}>
        <div style={{ padding:'18px 22px',borderBottom:'1px solid #1e2d3d',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
          <div>
        ...
        <div style={{ flex:1,overflowY:'auto',padding:'18px 22px' }}>
          ...
          {loading && (
            <div style={{ display:'flex',justifyContent:'flex-start',marginBottom:14 }}>
              <div style={{ background:'#0d1117',border:'1px solid #1e2d3d',borderRadius:'4px 14px 14px 14px',padding:'12px 16px',color:'#4a5c7a',fontSize:13 }}>
                &#x26A1; Thinking...
              </div>
            </div>
          )}
        ...
        <div style={{ padding:'14px 18px',borderTop:'1px solid #1e2d3d',display:'flex',gap:10 }}>
          <input
            style={{ ...S.inp, flex:1 }}
            value={input}
            onChange={e=>setInput(e.target.value)}
            onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&send()}
            placeholder="Type your thoughts... (Enter to send)"
            disabled={loading}
          />
    ...
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',zIndex:1000,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'24px 16px',overflowY:'auto' }}>
      <div style={{ background:'#0a0f1a',border:'1px solid #2d3f55',borderRadius:16,padding:28,width:'100%',maxWidth:820 }}>
        ...
        <div style={{ ...S.g4, marginBottom:14 }}>
          ...
          <div><label style={S.lbl}>Setup Type</label><select style={S.sel} value={f.setup} onChange={e=>set('setup',e.target.value)}>{SETUPS.map(s=><option key={s}>{s}</option>)}</select></div>
        </div>
        {isOpts ? (
          <div style={{ ...S.g4, marginBottom:14 }}>
        ...
        <div style={{ ...S.g4, marginBottom:14 }}>
          ...
          <div><label style={S.lbl}>Exit Price ($) — optional</label><input style={S.inp} type="number" step="0.01" value={f.exitPrice} onChange={e=>set('exitPrice',e.target.value)} placeholder="190.00" /></div>
        ...
        {pnl && (
          <div style={{ background:parseFloat(pnl)>=0?'rgba(16,185,129,0.1)':'rgba(239,68,68,0.1)', border:`1px solid ${parseFloat(pnl)>=0?'rgba(16,185,129,0.3)':'rgba(239,68,68,0.3)'}`, borderRadius:8,padding:'10px 14px',marginBottom:14 }}>
            ...
            <span style={{ color:parseFloat(pnl)>=0?'#10b981':'#ef4444',fontWeight:700,fontSize:15 }}>
              {parseFloat(pnl)>=0?'+':''}{pnl}
            </span>
          </div>
        )}
        <div style={{ marginBottom:14 }}>
          <label style={S.lbl}>Trade Thesis / Pre-trade notes</label>
          <textarea style={{ ...S.inp,minHeight:64,resize:'vertical' }} value={f.notes} onChange={e=>set('notes',e.target.value)} placeholder="What was the setup? Why did you enter? What were your targets?" />
        </div>
        <div style={{ marginBottom:20 }}>
          <label style={S.lbl}>Lesson Learned</label>
          <textarea style={{ ...S.inp,minHeight:56,resize:'vertical' }} value={f.lessonLearned} onChange={e=>set('lessonLearned',e.target.value)} placeholder="What did you learn from this trade? What would you do differently?" />
        </div>
...
function EntryCard({ entry, onChat }) {
  ...
  const emotionColors = { 'Confident':'#10b981','Disciplined':'#3b82f6','Patient':'#06b6d4','Cautious':'#f59e0b','FOMO':'#f97316','Fearful':'#ef4444','Greedy':'#f97316','Anxious':'#f43f5e' }
  const ec = emotionColors[entry.emotion] || '#4a5c7a'
  return (
      ...
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10 }}>
        <div>
          <div style={{ display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',marginBottom:4 }}>
            ...
            {entry.emotion && <span style={{ color:ec,fontSize:10 }}>{entry.emotion}</span>}
          </div>
          <div style={{ color:'#4a5c7a',fontSize:11 }}>
            {entry.entryDate}{entry.exitDate ? ' → '+entry.exitDate : ''} &bull; {entry.status === 'closed' ? 'Closed' : 'Open'}
          </div>
        </div>
        {entry.status === 'closed' && (
    ...
    <div style={S.page}>
      ...
      <div style={S.hdr}>
        <div>
          ...
          <div style={S.sub}>{entries.length} trades logged &bull; AI coach available after every trade</div>
        </div>
      ...
      {shown.length === 0 ? (
        <div style={{ ...S.card, textAlign:'center', padding:'56px 24px' }}>
          ...
          <div style={{ color:'#4a5c7a',fontSize:12,marginBottom:22,maxWidth:400,margin:'0 auto 22px' }}>
            Log your trades and chat with your AI journal coach. After every close, your coach asks: <em style={{color:'#8b9fc0'}}>"What was going through your head?"</em>
          </div>
          <button style={S.btn()} onClick={()=>setShowAdd(true)}>Log First Trade</button>
        </div>
      ) : (
        <div>
          {shown.map(e => <EntryCard key={e.id} entry={e} onChat={setChatEntry} />)}
        </div>
      )}
    </div>
  )
